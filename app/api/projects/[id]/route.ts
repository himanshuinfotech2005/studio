...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        return NextResponse.json(
            { error: "Failed to update project", message: error instanceof Error ? error.message : String(error) },
            { status: 500 }
        );
    }
}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        return NextResponse.json(
            { error: "Failed to delete project", message: error instanceof Error ? error.message : String(error) },
            { status: 500 }
        );
    }
}